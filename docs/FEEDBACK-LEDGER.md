...
|---|---|---|---|
| 2026-06-14 | **Wooden sword won't sell to the child.** Also: is it a *toy* or a *practice* sword? | **DECISION-NEEDED** → then build | Resolve the canon first (toy vs practice sword changes its sell value / who-buys-it / age-gate semantics), THEN fix the *sell-eligibility rule* class — an item declares `sellable` + `ageAllowed` + `value`, the shop reads it; don't special-case the wooden sword. Trigger: Van's toy-vs-practice call. Pairs with the shop-sell bug below. |
| 2026-06-14 | **Shops show NO item list and NO sell option on my screen.** Recurring — reported fixed before, still broken live. | **IN-PROGRESS (recurring — re-open)** | Class fix, not another patch: build a **runtime shop-render assertion** (boot fresh save → open each shop → assert the buy list AND the sell toggle are present + populated in Van's exact state). The `shop-stock-non-empty` gate checks DATA; it went green while Van's SCREEN was empty → the gate's promise is wider than its assertion (BUILD-METHOD verification doctrine). Widen to a live render check before claiming fixed again. Trigger: now. |
...
| 2026-06-14 | **The menu closes when I navigate sideways to Map** (should move to the Map tab, not exit). | **IN-PROGRESS (bug)** | Fix the **menu tab-navigation input class**: LEFT/RIGHT cycles tabs, only Esc/close exits — audit ALL tabs for the same swallow-and-close (not just Map). Trigger: now. |
