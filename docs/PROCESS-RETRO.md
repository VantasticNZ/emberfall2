...
| **The "exhaustive" table passed but Van's eyes failed on 3 new counts** | doors placed beside/on walls not in the painted opening; one door size for all; locked door looked open before the choice; avatar vanished in the dialog | the table never compared the door's **position/size to the painted ARTWORK** (it checked "a door sprite is in front" — not "in the painted opening, at the opening's size"); never asserted **avatar pixels DURING a dialog**; never checked **sequence ORDER** (choice-before-open) | New permanent cases: **door-in-painted-opening** (place+size to a rect MEASURED from the sprite PNG, per asset), **door-fits-rect**, **avatar-visible-in-dialog** (pixel-sample the player's screen pos while `_dlg` is open), **locked-sequence-order** (choice up + door still shut, no sfx, before open). Root of the avatar bug: the door at `baseDepth+2` over a band-centre threshold occluded the player → put the trigger at the FEET line so the player sorts in front. (Also: teleporting in a test can trigger a re-stream that DESTROYS sprite refs — re-fetch from the live list, or drive with real keys.) |
