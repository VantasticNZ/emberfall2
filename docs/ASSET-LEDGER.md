...
public/art/icons/   | single 32px CROPS from ElizaWy LPC Objects "Small Items" (Food/Bread A, Cheese A, Fruit B, Meals, Seafood A; Ores & Ingots/Ore, Iron; Lumber) via scripts/build_item_icons.py → bread/cheese/apple/stew/meat_pie/fish/iron_ore/bog_iron/timber | OGA-BY-3.0 | yes | inventory + buy/sell item icons (the rest of the catalogue falls back to its name)
```
