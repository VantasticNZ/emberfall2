...
## VERB DISPATCHER — HIT generalised, COLLECT/CARRY registry deferred (2026-06-15)
| Item | State | Why | Need / when |
|---|---|---|---|
| **Generic HIT verb** | ✅ DONE | `_applyHitReactions` reacts on ANY NPC (+ the hen) by sprite metadata, with escalation (protest→warning→break-off). Harness T10 proves it on a non-M2 NPC. | — (shipped 2f5cf4d7) |
| **Target→verb COLLECT/CARRY registry** | 🟡 DEFERRED | Eggs (COLLECT) + hen (CARRY) are still M2-bespoke. There is **no second COLLECT/CARRY target in the GH slice**, so a declarative registry has no second consumer yet — building it now is speculative infra, and it touches the sign-off-pending M2 verb wiring (RULE 1: spec + Van review before refactor). | The first NEW pick-up/carry target outside M2. Then build the registry to serve BOTH M2 and the new target (one dispatcher, not a one-off), with a full-expectation spec Van-reviewed first. |
